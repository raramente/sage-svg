...
const sageSvgSpriter = require('./util/sageSvgSpriter');
...
let webpackConfig = {
  ...
  plugins: [
    ...
    new sageSvgSpriter({
      svgSpriterRoot: path.resolve( config.paths.assets, config.paths.svgSprites ), // Base path for all the patterns
      defaultDest: path.resolve( config.paths.dist, config.paths.svgSprites ), // Folder to place the sprites
      files: config.svgSprites,
      log: 'verbose',
    }),
